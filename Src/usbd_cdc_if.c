...
/**
  ...
  * @retval None
  */
__weak void USBCDCRXCallback(uint8_t* Buf, uint32_t Len)
{
...
static int8_t CDC_Control_HS(uint8_t cmd, uint8_t* pbuf, uint16_t length)
{
  ...
  case CDC_SET_LINE_CODING:
  {
	  ...
	  if ( (req->bRequest) == (USB_REQ_TYPE_CLASS | USB_REQ_RECIPIENT_INTERFACE) )
	  {
		  host_port_open = true;
		  return (USBD_OK);
	  }
  ...
  {
	  USBD_SetupReqTypedef * req = (USBD_SetupReqTypedef *)pbuf;
	  if( ( (req->wValue) & USB_REQ_CLEAR_FEATURE) != 0)	//0x0001
	  {
		  // DTR is set, so Virtual Com port has been opened
		  host_port_open = true;
		  return (USBD_OK);
	  }
...
static int8_t CDC_Receive_HS(uint8_t* Buf, uint32_t *Len)
{
  ...
  return (USBD_OK);
	...
	do
	{
		result = USBD_CDC_SetRxBuffer(&hUsbDeviceHS, &Buf[0]);
	}
...
/*
	...
	//VCP_read(RX_FIFO.data,FIFO_SIZE);
...
//  }
  ...
  /*
  uint32_t myLen = (uint32_t)Len;
  uint32_t* myLenP = &myLen;
  while(CDC_TransmitCplt_HS(Buf, myLenP, CDC_IN_EP) != USBD_OK)
  {
	  my_HAL_Delay(20);
  }
  */
