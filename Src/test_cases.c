...
//          printf("Line: %d %s", __LINE__, USB_ReturnErrorCodeStr(ret));
    ...
    for (int testCtr = 0; testCtr < NR_TESTS; testCtr++)
    {
        ...
        sprintf(configFileName, "Confmem%d.cfg", testCtr);
        ...
        uint32_t lenMeasureFile = strlen("Cell;time_per_1000");
        ret = USB_OpenWriteFile(usbHandle, measureFileName, (uint8_t *) "Cell;WIP_Polling", &lenMeasureFile,
        ...
        for (int addrCtr = 0; addrCtr < MEM_SIZE_ADR; addrCtr++)
        {
            ...
            if (addrCtr % 100 == 0)
            {
                ...
                USB_ERROR usbError = USB_WriteData(usbHandle, pollingBuffer, &lenPollingBuff, true);
                if (usbError.m_ErrCode != MEM_NO_ERROR)
